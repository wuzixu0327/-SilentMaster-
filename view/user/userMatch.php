            ...
            <div class="echo-full-hero-content">
                <div class="row gx-5 sticky-coloum-wrap">
                    <div class="col-xl-8 col-lg-8" >
                        ...
                        <?php
                        require_once ('../../PhpSrc/Service/Match_index.php');
                        $news = new Match_index();
                        $news->ShowMassage(1);
                        ?>
                        <div class="echo-de-category-show-more-btn text-center">
                            <a id="loadMoreBtn" class="text-capitalize echo-py-btn" onclick="loadMoreNews()">更多</a>
                        </div>
                    </div>
...
<script>
    var currentPage = 1; // 初始化page值
    var hiddenInputValue = document.getElementById("myInput").value;
    var  count = Math.floor(hiddenInputValue/5)+1
        ...
        if(currentPage<=count){
            ...
            xmlhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    // 在更多按钮前面插入返回的内容
                    document.getElementById("newsContainer").insertAdjacentHTML('beforeend', this.responseText);

                    // 增加page值
                    currentPage++;

                    // 根据 success 的值执行相应的操作
                }
            };
            // // 发送 GET 请求到 loadMoreNews.php，包括当前的page值
            xmlhttp.open("GET", "../../PhpSrc/Controller/loadMoreMatch.php?page=" + currentPage, true);
            xmlhttp.send();
        }
        else {
            document.getElementById("loadMoreBtn").innerText = '到底了！';
        }
        // 发送 AJAX 请求
    }
</script>
